...
class DeepHedging(keras.Model):
    def __init__(self, dist_size, data_train, data_val, model_name="model"):
    ...
    def loss_function(self, X, batch_size):
        ...
        K = X[:, 0]   # <-- new: strike = this window's own starting price

        return tf.reduce_mean(tf.pow(self.price + strategy_pnl - self.payoff(X[:,-1], K), 2))
    ...
    def train_delta_strategy(self, epochs, batch_size):
        ...
        for epoch in range(epochs):
            ...
            for index in range(data_generator.__len__()):
                ...
                trainable_vars = self.delta_hedge.trainable_variables + [self.price]
            ...
            if loss_val < best_loss_val:
                ...
                self.delta_hedge.save_weights(f"{self.model_name}.weights.h5")

        print(self.trainable_variables[-1])
        self.delta_hedge.load_weights(f"{self.model_name}.weights.h5")
        print(self.trainable_variables[-1])
